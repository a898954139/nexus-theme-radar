...
## 台股題材雷達｜Theme → Symbol 靜態雷達 MVP

**保留 AI News Radar 的靜態站、GitHub Actions 與 JSON pipeline，將 AI relevance layer 改造成台股題材判定與候選股映射。**
...
```bash
python scripts/update_theme_radar.py --output-dir data --window-hours 72 --max-events 500 --max-candidates 200
```

也保留 deterministic sample data 供測試：

```bash
python scripts/generate_theme_demo.py --output-dir data --window-hours 48 --max-events 50 --max-candidates 20
```

公開後 GitHub Pages 會提供：
...
`tracking-candidates.json` 可直接作為 NexusDashboard 的 upstream query endpoint。
