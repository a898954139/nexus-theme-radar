const state = {
  ...
  allDedup: true,
  allDataLoaded: false,
  allDataUrl: "data/theme-events.json",
  allDataPromise: null,
  // Legacy fork compatibility: latest-24h.json remains a documented Pages payload, but this MVP front-end boots from theme-events.json.
  siteFilter: "",
  authorFilter: "",
  query: "",
...
// 單層題材 tab：全部（預設、不篩選）+ 市場題材分類，互斥單值。
const SECTION_DEFS = [
  { id: "all", label: "全部", short: "全部", description: "不篩選題材，查看全部市場訊號" },
  { id: "ai_infrastructure", label: "AI 基建", short: "AI 基建", description: "AI 運算與資料中心基礎設施" },
  { id: "semiconductors", label: "半導體", short: "半導體", description: "半導體製造、先進封裝與設備材料" },
  { id: "thermal_cooling", label: "散熱", short: "散熱", description: "散熱、液冷與相關零組件" },
  { id: "pcb_abf_hdi", label: "PCB / ABF", short: "PCB / ABF", description: "PCB、ABF 載板與 HDI 供應鏈" },
  { id: "memory_hbm", label: "記憶體", short: "記憶體", description: "記憶體、HBM 與相關供應鏈" },
  { id: "optical_cpo", label: "光通訊", short: "光通訊", description: "光通訊、矽光子與 CPO" },
  { id: "power_grid", label: "電源 / 重電", short: "電源 / 重電", description: "電源供應、重電與電網建設" },
  { id: "defense_drone", label: "國防 / 無人機", short: "國防 / 無人機", description: "國防、航太與無人機供應鏈" },
  { id: "robotics", label: "機器人", short: "機器人", description: "機器人、自動化與關鍵零組件" },
];

// UI 分類保持市場中立；此表只收斂目前資料產生器使用的題材別名。
const THEME_SECTION_COMPAT = Object.freeze({
...
function relatedSymbolLabels(item) {
  const related = Array.isArray(item.related_symbols) ? item.related_symbols : [];
  const labels = related.map((entry) => {
    if (typeof entry === "string") return entry;
    if (!entry || typeof entry !== "object") return "";
    return entry.symbol || entry.instrument_id || "";
  }).filter(Boolean);
  if (labels.length) return labels;
  return Array.isArray(item.related_symbol_codes) ? item.related_symbol_codes.filter(Boolean) : [];
}

function itemHaystack(item) {
  const themeSignals = (Array.isArray(item.matched_themes) ? item.matched_themes : [])
    .flatMap((theme) => [theme.theme_id, theme.name_zh, ...(Array.isArray(theme.signals) ? theme.signals : [])]);
  return [
    item.title,
    item.title_zh,
    item.title_en,
    ...
    item.primary_theme_id,
    ...themeSignals,
    ...relatedSymbolLabels(item),
...
function itemThemeSectionIds(item) {
  const primaryThemeId = String(item?.primary_theme_id || "").trim();
  const matchedThemeIds = (Array.isArray(item?.matched_themes) ? item.matched_themes : [])
    .map((theme) => (typeof theme === "string" ? theme : theme?.theme_id))
    .filter(Boolean);
  return [...new Set([primaryThemeId, ...matchedThemeIds])]
    .map((themeId) => THEME_SECTION_COMPAT[themeId])
    .filter(Boolean);
}

// 題材事件與追蹤候選共用 primary_theme_id + matched_themes 資料契約。
function itemSection(item) {
  return itemThemeSectionIds(item)[0] || "all";
}

function itemMatchesSection(item, sectionId = state.activeSection) {
  return !sectionId || sectionId === "all" || itemThemeSectionIds(item).includes(sectionId);
}
...
function storyMatchesSection(story, sectionId = state.activeSection) {
  const rep = storyRepresentativeItem(story);
  return !sectionId || sectionId === "all" || (rep && itemMatchesSection(rep, sectionId));
}
...
function renderItemNode(row) {
  ...
  const themeMatches = Array.isArray(item.matched_themes) ? item.matched_themes : [];
  if (themeMatches.length) {
    themeMatches.slice(0, 2).forEach((theme) => {
      const label = theme.name_zh || theme.theme_id;
      if (label) metaRow.appendChild(itemTagChip(label));
    });
    const relatedSymbols = relatedSymbolLabels(item);
    relatedSymbols.slice(0, 3).forEach((symbol) => {
      metaRow.appendChild(itemTagChip(`股 ${symbol}`));
    });
  } else {
    // Legacy AI records keep their existing section chip.
    metaRow.appendChild(itemTagChip(sectionBadgeLabel(itemSection(item))));
  }
...
async function init() {
  ...
  if (newsResult.status === "fulfilled") {
    ...
    state.allDataLoaded = Boolean(payload.items_all || payload.items_all_raw || payload.items);
