...
import { DeviceType, FinancialQuarter, InstitutionalFlowItem, StockFundamentals, StockTab } from '../../types';
...
const StockFlows: React.FC<{ flows: InstitutionalFlowItem[] }> = ({ flows }) => {
  ...
  return <div className="stock-flow-tab"><article className="stock-flow-chart"><div className="section-heading"><span className="page-kicker">INSTITUTIONAL FLOW · DAILY NET</span><span className="flow-chart-legend"><span><i className="flow-legend-bar" />日淨額</span><span><i className="flow-legend-line" />累計淨額</span><b>張</b></span></div><svg viewBox="0 0 600 160" preserveAspectRatio="none"><line x1="0" x2="600" y1="80" y2="80" className="chart-axis" />{chartFlows.map((flow, index, values) => { const x = values.length <= 1 ? 300 : index / (values.length - 1) * 570 + 15; const h = Math.abs(flow.total_net) / max * 62; return <g key={flow.date}><rect x={x - 10} y={flow.total_net >= 0 ? 80 - h : 80} width="20" height={h} className={flow.total_net >= 0 ? 'chart-bar-green' : 'chart-bar-red'} rx="2" /><text x={x} y="155" className="chart-label" textAnchor="middle">{flow.date.slice(5)}</text></g>; })}<polyline points={cumulativePoints} className="flow-cumulative-line" />{cumulative.map((value, index) => { const x = chartFlows.length <= 1 ? 300 : index / (chartFlows.length - 1) * 570 + 15; return <circle key={`${chartFlows[index]?.date}-cumulative`} cx={x} cy={cumulativeY(value)} r="2.5" className="flow-cumulative-dot"><title>{`${chartFlows[index]?.date} 累計 ${value.toLocaleString()} 張`}</title></circle>; })}</svg></article><div className="table-scroll"><table className="flow-detail-table"><thead><tr><th>日期</th><th>外資</th><th>投信</th><th>自營商</th><th>合計</th></tr></thead><tbody>{flows.map((flow) => <tr key={flow.date}><th className="mono-num">{flow.date}</th><td className={flow.foreign_net >= 0 ? 'positive mono-num' : 'negative mono-num'}>{flow.foreign_net.toLocaleString()}</td><td className={flow.trust_net >= 0 ? 'positive mono-num' : 'negative mono-num'}>{flow.trust_net.toLocaleString()}</td><td className={flow.dealer_net >= 0 ? 'positive mono-num' : 'negative mono-num'}>{flow.dealer_net.toLocaleString()}</td><td className={flow.total_net >= 0 ? 'positive mono-num' : 'negative mono-num'}>{flow.total_net.toLocaleString()}</td></tr>)}</tbody></table></div></div>;
};

export const StockDetail: React.FC<StockDetailProps> = ({ code, exchange, name, fundamentals, flows, device, initialTab, onBack, onTabChange }) => {
